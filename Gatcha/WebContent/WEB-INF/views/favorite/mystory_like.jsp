...
<%@ include file="/WEB-INF/inc/sidebar/sidebar_favorite.jsp" %>	
		...
		<div class="container col-sm-10 col-md-10">
			...
			<div class="row">
			<c:choose>
			<c:when test="${fn:length(ev_list_like) > 0 }">
				<c:forEach var="ev_list_like" items="${ev_list_like}">
				<div class="col-xs-6 col-sm-4 col-md-3 col-lg-3">
					<div class="img" style="background-image: url('${ev_list_like.poster}')"> 
						<div class="wrapper">
							<div class="image-wrapper">
								<a data-toggle="modal" href='${pageContext.request.contextPath}/evaluate/evaluate_sub.do?kfa_id=${ev_list_like.kfaId}' data-toggle="modal" data-target="#movie_info_modal" >
									<span class="clickable"></span>
								</a>
										<a id="hatetosee" class="hatetosee"><i class="glyphicon glyphicon-remove-circle"></i></a>
										<input type="hidden" id="hiddenvalue" value="${ev_list_like.kfaId}"/>
										<a id="alreadyhatetosee" class="alreadyhatetosee hide"><i class="glyphicon glyphicon-remove-circle"></i></a>
							</div>
							<div class="cation-wrapper1">
								<h4>${ev_list_like.title}</h4>
								<hr/>
								<br/>
								<div id="mystarpoint${ev_list_like.kfaId}"></div>
								<input type="hidden" id="hiddenstarpoint" value="${ev_list_like.myStarpoint}"/>
								<br/>
								<c:if test="${ev_list_like.like==null or ev_list_like.like=='0'}">
								<span>
								...
								<a class="btn btn-link" href='${pageContext.request.contextPath}/evaluate/evaluate_diary.do?kfa_id=${ev_list_like.kfaId}' data-toggle="modal" data-target="#diary_edit_modal">
...
// 	$(document).trigger('reset'); 
	...
	<c:forEach var="ev_list_like" items="${ev_list_like}">
	$('#mystarpoint${ev_list_like.kfaId}').raty({
		cancel: false,
		half: true,
		score: $('#mystarpoint${ev_list_like.kfaId}').next().val(),
        ...
        click : function(score){
        	var mykfaid = ${ev_list_like.kfaId};
        	var myscore =''+score;
        	alert("별점이 등록되었습니다.");
