 ...
 */
//좋아요 기능 
@WebServlet("/evaluate/evaluate_dislike.do")
public class EvaluateDislike extends BaseController {
...
/** 버튼 이벤트를 처리하기 위해 만들었던 서블릿으로서, 이벤트 실행시 항목이 있는지 selectEvaluateCount로 찾아내어 1이면 updateEvaluateChoice로 들어가게 하였습니다. 0인경우에는 
   * insertEvaluateMayNoOne가 실행되어 평가를 추가할것입니다.  */
	...
	private static final long serialVersionUID = 3447345755452162851L;
	/** 1)사용하고자 하는 객체들 선언 */
	WebHelper web;
	Logger logger;
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result < 1){
			...
			try{
				evaluateService.updateEvaluateDislike(evaluateNext);
			}catch(Exception e){
				e.getLocalizedMessage();
